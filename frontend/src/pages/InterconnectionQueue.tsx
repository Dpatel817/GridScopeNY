        ...
        <ReBarChart data={data} layout="vertical" margin={{ top: 4, right: 60, left: 90, bottom: 4 }}>
          ...
          <Bar dataKey={yKey} radius={[0, 4, 4, 0]} maxBarSize={24}>
            ...
            <LabelList dataKey={yKey} position="right"
              formatter={(v: unknown) => typeof v === 'number' ? `${Math.round(v).toLocaleString()}` : ''}
...
const SHEET_LABELS: Record<string, string> = {
  active: 'Active Queue', cluster: 'Cluster Projects',
  affected_system: 'Affected System', in_service: 'In Service',
  withdrawn: 'Withdrawn', cluster_withdrawn: 'Cluster Withdrawn',
...
export default function InterconnectionQueue() {
  ...
  const summary: SummaryData | null = summaryData?.data?.length ? parseSummary(summaryData.data as any[]) : null;
  ...
  const kpis = useMemo(() => {
    ...
    return {
      ...
      storageMw, solarMw, windMw, largest, avgSize, totalMw,
  ...
  const intelligenceSummary = useMemo(() => {
    ...
    parts.push(`The NYISO interconnection queue contains ${(kpis.activeCount + kpis.clusterCount).toLocaleString()} active and cluster projects totaling ${Math.round(totalPipelineMw).toLocaleString()} MW of proposed capacity.`);
    const topFuels = fuelChartData.slice(0, 3);
    if (topFuels.length) parts.push(`Dominant technologies: ${topFuels.map(f => `${f.Fuel} (${f.MW.toLocaleString()} MW)`).join(', ')}.`);
    const topZones = [...zoneChartData].sort((a, b) => b.MW - a.MW).slice(0, 3);
    if (topZones.length) parts.push(`Zones with the most queued capacity: ${topZones.map(z => `Zone ${z.Zone} (${z.MW.toLocaleString()} MW)`).join(', ')}.`);
    if (kpis.largest) parts.push(`Largest project: ${kpis.largest.project_name || kpis.largest.queue_pos} at ${Math.round(Number(kpis.largest.sp_mw)).toLocaleString()} MW (${fuelLabel(kpis.largest.fuel_type)}, Zone ${kpis.largest.zone}).`);
    if (kpis.inServiceMw > 0) parts.push(`${kpis.inServiceCount.toLocaleString()} projects (${Math.round(kpis.inServiceMw).toLocaleString()} MW) have reached in-service status.`);
    const newCount = Number(summary?.new_since_last || 0);
    if (newCount > 0) parts.push(`${newCount} new project(s) added since the last scrape.`);
    return parts.join(' ');
    ...
    <div className="page">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p className="page-subtitle">NYISO generation and storage interconnection pipeline — capacity analytics, technology trends, and project tracking</p>
        </div>
        <div className="iq-header-actions">
          <button className={`btn btn-primary iq-scrape-btn${scraping ? ' refreshing' : ''}`} onClick={handleScrape} disabled={scraping}>
      ...
      <div className="kpi-section">
        ...
        <div className="kpi-grid-fixed">
          ...
          <div className="kpi-card-fixed"><div className="kpi-label">Largest Project</div><div className="kpi-value">{kpis.largest ? <>{Math.round(Number(kpis.largest.sp_mw)).toLocaleString()}<span className="kpi-unit">MW</span></> : '—'}</div>{kpis.largest && <div className="kpi-sub">{kpis.largest.project_name ? String(kpis.largest.project_name).slice(0, 23) : String(kpis.largest.queue_pos)}</div>}</div>
        </div>
      </div>
        ...
        <div key="activity">
          <Widget draggable title="Recent Queue Activity" badge={newCount + removedCount > 0 ? `${newCount + removedCount} changes` : undefined} noPad>
            <div className="iq-activity-summary" style={{ padding: '12px 16px' }}>
              <div className="kpi-card" style={{ flex: 1 }}><div className="kpi-label">New Projects</div><div className="kpi-value">{newCount}</div></div>
              <div className="kpi-card" style={{ flex: 1 }}><div className="kpi-label">Projects Withdrawn</div><div className="kpi-value">{removedCount}</div></div>
              <div className="kpi-card" style={{ flex: 1 }}><div className="kpi-label">MW Added</div><div className="kpi-value">{Math.round(newMwAdded).toLocaleString()}<span className="kpi-unit">MW</span></div></div>
            </div>
            {changes.length > 0 ? (
          ...
          <Widget draggable title="Full Queue Table" badge={`${allRows.length} projects`} noPad>
            <div className="iq-table-header">
              <div className="iq-tabs">
                {sheetTabs.map(t => (
                  <button key={t.key} className={`iq-tab${sheetFilter === t.key ? ' active' : ''}`} onClick={() => setSheetFilter(t.key)}>
                    {t.label} <span className="iq-tab-count">{t.count}</span>
                  </button>
                ))}
              </div>
              <div className="iq-filter-row">
                ...
                <input type="text" className="iq-search" placeholder="Search projects, developers, counties..."
                ...
                <thead>
                  <tr>
                    {DISPLAY_COLS.map(col => {
                      ...
                      return (
                        <th key={col.key} style={{ minWidth: col.width, cursor: isSortable ? 'pointer' : 'default' }}
                          onClick={() => isSortable && handleSort(col.key as SortKey)}>
                          {col.label}
                          {sortKey === col.key && <span className="iq-sort-icon">{sortDir === 'asc' ? ' ▲' : ' ▼'}</span>}
                        </th>
                      );
                    })}

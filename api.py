...
_etl_logger = logging.getLogger("api.etl")


def _run_etl_fetch(lookback_days=7):
    from etl.datasets import get_dated_datasets, get_snapshot_datasets
    from etl.fetchers import create_session, fetch_daily_file, fetch_snapshot, get_date_range
    ...
    dated = get_dated_datasets()
    for name, meta in sorted(dated.items()):
        try:
            raw_paths = []
            for d in dates:
                path = fetch_daily_file(session, meta, d, name)
                if path:
                    raw_paths.append(path)
            if raw_paths:
                fetched += len(raw_paths)
                _etl_logger.info(f"Fetched {len(raw_paths)} files for {name}")
        except Exception as e:
            _etl_logger.error(f"Fetch error for {name}: {e}")
            errors.append(f"{name}: {e}")

    snapshots = get_snapshot_datasets()
    for name, meta in sorted(snapshots.items()):
        try:
            path = fetch_snapshot(session, meta, name)
            if path:
                fetched += 1
                _etl_logger.info(f"Fetched snapshot for {name}")
        except Exception as e:
            _etl_logger.error(f"Fetch error for {name}: {e}")
            errors.append(f"{name}: {e}")

    session.close()
    return {"fetched_files": fetched, "errors": errors}


def _run_etl_process(lookback_days=7):
    from etl.datasets import get_dated_datasets, get_snapshot_datasets
    from etl.fetchers import get_date_range
    ...
    for name, meta in sorted(dated.items()):
        try:
            ...
            raw_subdir = RAW_DATA_DIR / name / ext
            if not raw_subdir.exists():
                continue
            raw_paths = sorted(raw_subdir.glob(f"*"))
            raw_paths = [p for p in raw_paths if p.is_file()]
            if not raw_paths:
                continue
            df = process_raw_files(raw_paths, meta, name)
            if not df.empty:
                upsert_parquet(df, name, meta)
                sync_to_legacy(name, meta)
                mark_dates_processed(name, dates)
                processed += 1
                _etl_logger.info(f"Processed {name}: {len(df)} rows")
        except Exception as e:
            _etl_logger.error(f"Process error for {name}: {e}")
            errors.append(f"{name}: {e}")
    ...
    for name, meta in sorted(snapshots.items()):
        try:
            if meta["dataset_type"] == "snapshot_xlsx":
                raw_subdir = RAW_DATA_DIR / name / "xlsx"
            else:
                raw_subdir = RAW_DATA_DIR / name / "csv"
            if not raw_subdir.exists():
                continue
            raw_files = sorted(raw_subdir.glob("*"))
            raw_files = [p for p in raw_files if p.is_file()]
            if not raw_files:
                continue
            path = raw_files[-1]
            if meta["dataset_type"] == "snapshot_xlsx" and name == "interconnection_queue":
                from ETL.fetch_interconnection_queue import SHEET_MAP, COLUMN_MAP
                try:
                    xls = pd.ExcelFile(path, engine="openpyxl")
                except Exception as e:
                    _etl_logger.error(f"Cannot read queue xlsx: {e}")
                    continue
                frames = []
                for sheet_name in xls.sheet_names:
                    ...
                    if not source_sheet:
                        continue
                    try:
                        sdf = pd.read_excel(xls, sheet_name=sheet_name, engine="openpyxl")
                        sdf.columns = sdf.columns.str.strip().str.lower()
                        rename = {k: v for k, v in COLUMN_MAP.items() if k in sdf.columns}
                        sdf = sdf.rename(columns=rename)
                        sdf["source_sheet"] = source_sheet
                        frames.append(sdf)
                    except Exception as e:
                        _etl_logger.warning(f"Sheet {sheet_name}: {e}")
                if frames:
                    combined = pd.concat(frames, ignore_index=True)
                    upsert_parquet(combined, name, meta)
                    sync_to_legacy(name, meta)
                    processed += 1
            else:
                ...
                if not df.empty:
                    ...
                    sync_to_legacy(name, meta)
                    processed += 1
            mark_snapshot_fetched(name)
        except Exception as e:
            _etl_logger.error(f"Process error for {name}: {e}")
            errors.append(f"{name}: {e}")

    return {"processed_datasets": processed, "errors": errors}
...
def refresh_data():
    ...
    try:
        from src.api_data_loader import _df_cache
        _df_cache.clear()
        fetch_result = _run_etl_fetch()
        process_result = _run_etl_process()
        _df_cache.clear()
        fetch_ok = len(fetch_result["errors"]) == 0
        process_ok = len(process_result["errors"]) == 0
        return {
            "status": "ok" if fetch_ok and process_ok else "partial",
            "fetch": "ok" if fetch_ok else "error",
            "process": "ok" if process_ok else "error",
        }
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
    finally:
...
def run_etl_process():
    try:
        result = _run_etl_process()
        return {
            "status": "ok" if len(result["errors"]) == 0 else "error",
            "processed_datasets": result["processed_datasets"],
            "errors": result["errors"][-5:] if result["errors"] else [],
        }
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
